...
#2026.6.27 Beta0.64测试版本：新增了language和restart命令，目前可支持更换系统语言与重载工具以解决卡死问题
...
silent_link_verify() {
    ...
    local commands=("about" "list" "network" "disk" "ctime" "process" "helpUCST" "nkill" "open" "delete" "new" "mod" "driver" "backup" "check" "UCST" "download" "search" "account" "sai" "amend" "language" "restart") #验证命令列表
...
command_sai() {
    local config_file="$HOME/.ucst_sai_config"
    for cmd in curl jq md5sum; do #检查依赖，所有的依赖都会在全局依赖系统中完成安装
        if ! _cmd_exists "$cmd"; then
            echo "错误: 缺少命令 '$cmd'"
            echo "正在尝试安装"
    ...
    if [ ! -f "$config_file" ]; then #检查配置
        echo "错误: 未找到配置文件"
        echo "请先运行:account -s完成账户与CA配置"
        return 1
    fi
    local api_url=$(grep '^api_url=' "$config_file" | cut -d"'" -f2 2>/dev/null) #读取已经被配置的配置
    ...
    if [ "$http_code" = "200" ]; then
        ...
        if [ -n "$session_id" ] && [ "$session_id" != "null" ]; then
            ...
            echo "无法获取会话ID, 使用无会话模式（假会话）"
            session_id=""
        fi
    else
        echo "会话创建失败, 使用无会话模式（假会话）"
        session_id=""
    fi
    rm -f "$session_file"
    ...
    while true; do
        ...
        case "$prompt" in
            ...
            switch)
                if [ "$mode" = "chat" ]; then
                    mode="coder"
                    echo "已切换到 SAI-Coder 模式"
                else
                    mode="chat"
                    echo "已切换到 SAI-Chat 模式"
                fi
                continue
                ;;
            "")
        ...
        local token=$(_generate_token_for_api "$prompt") #生成token（规则在下方）
        echo "正在思考"
        local json_data="" #构建请求
        local api_endpoint=""
        if [ "$mode" = "chat" ]; then
        ...
        http_code=$(curl -s -X POST \
            ...
            -o "$temp_file" \
            -w "%{http_code}")
        if [ "$http_code" = "200" ]; then #处理响应
            local raw_response=$(cat "$temp_file")
            if echo "$raw_response" | jq -e . >/dev/null 2>&1; then #用JSON解析
                if echo "$raw_response" | jq -e '.output' >/dev/null 2>&1; then #JSON？纯度如何
                    ...
                    if echo "$inner_json" | jq -e . >/dev/null 2>&1; then
                        if echo "$inner_json" | jq -e '.output' >/dev/null 2>&1; then #JSON？666给我的高人瞎司勒
            ...
            if [ "$http_code" = "401" ] || [ "$http_code" = "403" ]; then
                echo "认证失败，建议运行account -s更新CA"
            fi
        fi
        rm -f "$temp_file"
        echo ""
    done
    if [ -n "$session_id" ]; then #清理会话
        curl -s -X POST \
            "${api_url}/api/session/delete" \
            -H "Content-Type: application/json" \
            -H "ca: $ca" \
...
_generate_token_for_api() { #TOKEN生成
    local prompt="$1"
